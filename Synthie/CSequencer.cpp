...
bool CSequencer::Generate(double* frame)
{
    ...
    while (m_currentNote < (int)m_notes.size())
    {
        ...
        else if ((note->Instrument() == L"RecordingInstrument"))
        {
            instrument = new CRecordingInstrument();
        }

        // Configure the instrument object
        if (instrument != NULL)
        {
            instrument->SetSoundSource(note->SoundRecording());
            instrument->SetBPM(m_bpm);
            instrument->SetEffects(m_effects);
...
void CSequencer::XmlLoadInstrument(IXMLDOMNode* xml)
{
...
void CSequencer::XmlLoadNote(IXMLDOMNode* xml, std::wstring& instrument)
{
    m_notes.push_back(CNote());
    m_notes.back().XmlLoad(xml, instrument);
}

void CSequencer::XmlLoadEffect(IXMLDOMNode* xml, std::wstring& instrument)
{
    m_notes.push_back(CNote());
    m_notes.back().XmlLoad(xml, instrument);
}